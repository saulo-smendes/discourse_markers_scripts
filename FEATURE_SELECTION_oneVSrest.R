...
for (target_dm in classes) {
  ...
  # Resample data randomly to get the same amount of OTHER class in iu as ALL class
  # Get the number of ALL class in iu
  n_target <- length(which(data$iu == target_dm))
  
  # Get the number of OTHER class in iu
  n_other <- length(which(data$iu == "OTHER"))
  
  # Get the indices of OTHER class in iu
  other_indices <- which(data$iu == "OTHER")
  
  # Sample randomly from OTHER class in iu
  other_indices <- sample(other_indices, n_target)
  
  # Get the indices of ALL class in iu
  target_indices <- which(data$iu == target_dm)
  
  # Combine indices of ALL and OTHER class in iu
  indices <- c(target_indices, other_indices)
  
  # Get the data with the same amount of ALL and OTHER class in iu
  data <- data[indices, ]
  ...
  png(file = name_cp, width = 800, height = 600, units = "px", res = 400)
  ...
  png(file=name_plot, width=700, res=400)
  par(las = 2)
  par(mar = c(15, 6, 2, 1))
  ...
  for (n in 1:nmod){
    ...
    conf <- confusionMatrix(mTag, lUDdat$Tag)
  ...
  # Evaluate the model
  conf_matrix <- confusionMatrix(predictions, data_subset$Tag)
  print(conf_matrix)
  
  # Plot the Decision Tree
  name_dt <- paste("dt_plot_", target_dm)
  name_dt <- paste(name_dt, ".png")
  png(file = name_dt, width = 800, height = 600, units = "px", res = 400)
