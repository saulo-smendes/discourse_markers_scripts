...
data <- read.csv("C:/Users/Saulo Mendes Santos/OneDrive/Documents/2. LETRAS/0. Doutorado/0. Recherche/1.5. Classification/data_smote.csv")
...
plot(RegsS, summary(REGS)$cp, col = "#008810", log = "y",
     xlab = "Number of parameters (SMOTE)", main="Cp stat", ylab = "Cp Stat", cex.lab = 1.2)
...
barplot(sort(colSums(RegsW)/nrow(RegsW)*100, decreasing = TRUE)[-1], col = 3, ylim = c(0, 100),
        ylab = "Number of times the variable was select (%)", main="Most selected features (SMOTE)", cex.lab = 1.2)
nmod <-nrow(RegsW)
...
plot(1:nv, maxoa, col = 1, type = "b", ylim = c(0.5, 1), xlab = "Number of parameters",
     ylab = "Max accuracy score (%)", main="Max accuracy score by n of parameters", cex.lab = 1.2, frame.plot = FALSE, xlim = c(0, 27))
